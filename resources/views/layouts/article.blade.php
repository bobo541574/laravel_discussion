    ...
    <div class="card shadow-lg mb-2" id="card_{{ $article->id }}">
        <div class="card-body pt-2">
            ...
            <div class="row card-title mb-0">
                <div class="col">
                    <a href="{{ route('articles.timeline', $article->user->id) }}" class="card-link">
                        <img src="{{ asset($article->user->photo) }}" class="mr-3 rounded-circle" width="45px"
                            height="45px" alt="user_img">
            ...
            <p class="card-text" id="read_more_{{ $article->id }}">{{ substr($article->body, 0, 150) }}... <a
                    href="javascript:void(0)" class="card-link" onclick="readMore({{ $article }})">Read
                    More</a></p>
        </div>
        <div class="card-footer">
            ...
            <a href="{{ route('articles.show', $article->id) }}" class="btn btn-sm btn-light comment"
                data-placement="top" title="comment" data-toggle="tooltip" onclick="comment({{ $article->id }})">
                <i
                    class="fas fa-comment-alt
                @auth
                    @foreach ($article->comments as $comment) {{ $comment->user->id == auth()->user()->id ? 'text-info' : '' }} @endforeach
                @endauth
            ">
